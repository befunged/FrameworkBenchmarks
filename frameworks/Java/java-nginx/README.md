...
3. Create `java-nginx.dockerfile`
...
You can create multiple implementations and they will all conform to `[name in benchmark_config.json].dockerfile`. For example, the `default` implementation in `benchmark_config.json` will be `java-nginx.dockerfile`, but if you wanted to make another implementation that did only the database tests for MySQL, you could make `java-nginx-mysql.dockerfile` and have an entry in your `benchmark_config.json` for `java-nginx-mysql`.

4. Test your application

        $ tfb --mode verify --test java-nginx
...
Edit `.travis.yml` to ensure that Travis-CI will automatically run our verification tests against your new test. This file is kept in alphabetical order, so find where `TESTDIR=Java/java-nginx` should be inserted under `env > matrix` and put it there.
