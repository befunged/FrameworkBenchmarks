...
3. Create `fast-cgi-c.dockerfile`
...
You can create multiple implementations and they will all conform to `[name in benchmark_config.json].dockerfile`. For example, the `default` implementation in `benchmark_config.json` will be `fast-cgi-c.dockerfile`, but if you wanted to make another implementation that did only the database tests for MySQL, you could make `fast-cgi-c-mysql.dockerfile` and have an entry in your `benchmark_config.json` for `fast-cgi-c-mysql`.

4. Test your application

        $ tfb --mode verify --test fast-cgi-c
...
Edit `.travis.yml` to ensure that Travis-CI will automatically run our verification tests against your new test. This file is kept in alphabetical order, so find where `TESTDIR=C/fast-cgi-c` should be inserted under `env > matrix` and put it there.
...
initial fast cgi code and setup taken from
